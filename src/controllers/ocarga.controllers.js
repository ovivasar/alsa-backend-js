...
const obtenerTodasOCargasPlanCrossTab1 = async (req, res, next) => {
    ...
    strSQL = strSQL + " 'SELECT (ref_documento_id || ''-'' || ref_razon_social)::varchar(300) as ref_razon_social, TO_CHAR(fecha, ''YYYY-MM'') AS mes, count(cantidad) AS cantidad";
...
const obtenerTodasOCargasPlanCrossTab2Det = async (req, res, next) => {
    //Version analizado, similar formato excel manejado en administracion
    let strSQL;
    const { fecha_ini, fecha_proceso,codigo } = req.params;
...
const obtenerTodasOCargasPlanCrossTab4 = async (req, res, next) => {
    //Version analizado, similar formato excel manejado en administracion
    let strSQL;
    const { fecha_ini, fecha_proceso } = req.params;
    ...
    strSQL = "SELECT ";
    strSQL = strSQL + " vendedor as descripcion";
    ...
    strSQL = strSQL + " FROM crosstab(";
    strSQL = strSQL + " 'SELECT mve_venta.vendedor, TO_CHAR(mst_ocarga_detalle.fecha, ''YYYY-MM'') AS mes, (coalesce(sum(mst_ocarga_detalle.e_peso01),0)+coalesce(sum(mst_ocarga_detalle.e_peso02),0)+coalesce(sum(mst_ocarga_detalle.e_peso03),0)) AS cantidad";
    ...
    strSQL = strSQL + " AND mst_ocarga_detalle.tipo = ''E''";
    strSQL = strSQL + " AND (NOT mst_ocarga_detalle.ref_cod is null)"; //Filtrar solo ventas
    strSQL = strSQL + " GROUP BY mes, mve_venta.vendedor";
    strSQL = strSQL + " ORDER BY mve_venta.vendedor',";

    // Agrega los valores de los meses dinámicamente
    strSQL = strSQL + " $$VALUES " + meses.map((mes) => `('${mes}')`).join(', ') + "$$";
    strSQL = strSQL + " ) AS ct (vendedor text";
...
module.exports = {
    ...
    obtenerTodasOCargasPlanCrossTab2Det,
